/**
 * CoreShop SubscriptionBundle Subscription List
 *
...
const LIST_TYPE = 'coreshop_subscription'

const SubscriptionListInner: React.FC = () => {
  ...
  const listingBuilder = container.get<ObjectListingBuilder>('CoreShop/Subscription/Listing/Builder')
  ...
  React.useEffect(() => {
    const fetchFolderConfig = async (): Promise<void> => {
      try {
        const response = await fetch('/pimcore-studio/api/coreshop/subscription/get-folder-configuration')
        ...
        void messageApi.error('Failed to fetch subscription folder configuration')
      } finally {
        setLoading(false)
      }
    }
  ...
  if (folderId === null) {
    return (
      <div className={styles.loading}>
        <span>{t('coreshop_no_folder_configured', { defaultValue: 'No folder configured for subscriptions' })}</span>
      </div>
    )
  }
 ...
 *
 * Displays CoreShopSubscription DataObjects using Pimcore's DataObject listing.
 */
